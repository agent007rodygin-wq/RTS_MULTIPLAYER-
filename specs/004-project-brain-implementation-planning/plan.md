# Feature 004 - Project Brain Implementation Planning

**Feature Branch**: `[004-project-brain-implementation-planning]`
**Status**: Draft

> Non-authoritative migration note: the implementation-roadmap content was
> initially drafted while Feature 003 still held the planning material. That
> draft is not recoverable exactly from local source history, so Feature 004 is
> now the authoritative home for implementation planning. Feature 003 remains
...
## 3. Implementation Package Matrix
...
- A establishes canonical identity and registry authority.
- B defines canonical storage and lifecycle transitions.
- C defines the Brain Engine boundary and internal reasoning roles.
- D builds frozen context packages from canonical sources.
- E produces deterministic generation attempts.
- F validates outputs and gates publication.
- G publishes read-only Mirror snapshots.
- H stores structured investigation memory as a derived artifact.
- I orchestrates execution providers and review providers without authority drift.
- J coordinates repeatable operational workflows.
- K gates canonical publication and governance history.
- L reports failures and recovery evidence across the system.
...
## 6. Hard and Soft Dependencies

| Package | Hard Dependencies | Soft Dependencies |
| --- | --- | --- |
| A | none | traceability index |
| B | A | traceability, validation |
| C | A, B | traceability, validation |
| D | A, B, C | mirror, debug memory |
| E | C, D | validation |
| F | A, B, C, E | mirror, debug memory |
| G | A, C, E, F | debug memory, workflows |
| H | A, F, G | agents, workflows |
| I | C, F, H | mirror |
| J | I, F, G, H | mirror, debug memory |
| K | F, J | traceability |
| L | F, G, H, I, J, K | mirror, debug memory |
...
- Roll out by wave, not by package size.
- Do not start a downstream wave until the prior wave meets its exit criteria.
- Keep each rollout slice small enough to revert without affecting later waves.
- Publish only after validation, traceability, and governance gates agree.
- Prefer internal verification before any broader integration review.
...
The following decisions are intentionally deferred and are not architecture
changes:
...
## 16. Traceability to Feature 003

Feature 004 must reference, not duplicate, the Feature 003 baseline. The roadmap
is traceable to these Feature 003 sections:
...
- Mirror Contract;
- Debug Manifest;
- Agent and Ralph boundaries;
- Operational Workflows;
- Final Invariant Index;
- Final Open Decision Register;
- Implementation Package Map;
- Architecture Final Review verdict.

Feature 004 operationalizes those decisions into a roadmap; it does not change
them.
