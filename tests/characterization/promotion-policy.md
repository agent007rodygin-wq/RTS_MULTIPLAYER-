...
Define how a scenario moves from investigation to a permanent characterization
contract.
...
A proposed scenario may move to `CURRENT_ACCEPTED_BEHAVIOR` or
`LEGACY_COMPATIBILITY_BEHAVIOR` only after all of the following are true:
...
- `UNCONFIRMED_RUNTIME_BEHAVIOR` stays out of the permanent suite until it is
  promoted.
- `KNOWN_BUG_DO_NOT_FREEZE` never enters the permanent suite until a separate
  fix decision exists.
