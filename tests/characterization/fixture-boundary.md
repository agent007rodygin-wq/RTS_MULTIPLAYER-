...
Define the local deterministic data boundary for future characterization work.
...
- Fixtures can include record snapshots, ordered event traces, and expected
  outcomes.
- Fixtures must reset cleanly between runs.
- Fixtures must not require live PocketBase mutation.
...
- any state that only exists because production code was modified for the
  fixture

## Reset Policy

- Every run starts from the same fixture state.
- Running the same command twice without changing fixtures must produce the
  same result.
