...
async function fetchQueryRecords(desc: QueryDescriptor): Promise<RecordModel[]> {
  ...
  if (isZoneInQuery) {
    ...
    return typeof desc.maxItems === 'number' ? merged.slice(0, desc.maxItems) : merged;
  }
  ...
  if (desc.filter || (typeof desc.maxItems === 'number' && desc.maxItems > 0)) {
    ...
    const pageSize = typeof desc.maxItems === 'number' && desc.maxItems > 0
      ...
      : defaultPageSize;
    return fetchPagedRecords(desc.col, pageSize, fetchOptions, `${desc.col} filtered query`, desc.maxItems);
  }
...
export function onSnapshot(
  ...
  fetchQueryRecords(desc)
    .then((records) => {
      if (!destroyed) {
        ...
        callback(buildSnapshot(records as RecordModel[]));
      }
    })
    .catch((err) => {
  ...
  safeSubscribe(desc.col, '*', async (event) => {
    if (queriedZoneIds) {
      ...
      const eventRecordId = String(event?.record?.id || '').trim();
